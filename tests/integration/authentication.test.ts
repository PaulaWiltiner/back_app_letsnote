...
describe("POST /signIn", () => {
  ...
  describe("when body is valid", () => {
    ...
    it("should respond with status 404 if there is no user for given email", async () => {
    ...
    describe("when credentials are valid", () => {
      ...
      it("should respond with user data", async () => {
        const body = generateValidBody();
        const user = await createSession(body);

        const resp = await server.post("/signIn").send(body);
